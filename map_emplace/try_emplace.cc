...
int main()
{
    ...
    // Add an element and create an object in it
	if (auto [iter, inserted] = fridges.try_emplace("Dairy fridge", 5, false, false);inserted) {
		std::cout << "Inserted new element\n";
	} else {
		std::cout << "Element already existed, did not insert\n";
	}
